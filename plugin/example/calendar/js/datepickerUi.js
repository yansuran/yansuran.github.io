(function ($, DateCore, dateLanguage) {
    ...
    Calendar.prototype.calendarShow = function (mode) {
       ...
       //$(inputDom).last().after(dateboxFrame); nicky
	      ...
	      $(inputDom).parent('.timeSelectBar').next('.timeHideModel').show().find('.detailDateCont').append(dateboxFrame);
        ...
        if (me.options.trigger !== 'auto') {
            //绑定document关闭事件
            $(document).bind('click.clearDom', function (event) {
                var elem = $(event.target);
                if (elem.closest('.' + me.options.rootNode + '').length === 0 && elem.closest(me.$element.selector).length === 0 && elem.closest('*[data-trigger = "spcalendar"]').length === 0) {
                    me.calendarClose();
                }
            });
        }
    ...
    function loadDate(dateboxFrame, DateCore, year, month, date) {
       ...
       // var dateBoxDOM = me.$element.last().nextAll('.' + me.options.rootNode + '');
		var dateBoxDOM = me.$element.parent('.timeSelectBar').next('.timeHideModel').find('.detailDateCont'); //nicky
    ...
    function generateDateTemplate(templateArray, DateCore, year, month, date) {
        ...
        templateArray.push('<table class="table-condensed table-condensed-date">');
    ...
    function generateMonthTemplate(templateArray, DateCore, year, month) {
        ...
        templateArray.push('<table class="table-condensed table-condensed-month">'); //nicky
        templateArray.push('<thead><tr><th colspan="6" class="switch"><span class="switch-year">' + year + me.$Lang.str_year + '</span></th></tr>');
        templateArray.push('</thead>');
        templateArray.push('<tbody>');
        for (var m = 0, k = 0; m < data_Month.length / 6; m++) {
            ...
            for (var n = 0; n < 6; n++) {
                ...
                templateArray.push('<td width="16.666%" class="month ' + 'waiting' + '" title="' + tempString + '">' + me.$Lang.months[data_Month[k] - 1] + '</td>');
                k++;
            }
            templateArray.push('</tr>');
        }
    ...
    function generateWeekTemplate(templateArray, DateCore, year) {
        ...
        templateArray.push('<table class="table-condensed table-condensed-week">');
